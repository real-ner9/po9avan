export const REPLIES = {
  REGISTRATION: {
    INTRO:
      '👋 Привет! Давай создадим твой профиль, чтобы я мог найти для тебя ментора.',
    NICKNAME: 'Придумайте любой ник (до 24 символов). Можно латиницу и цифры.',
    PROFESSION: 'Какая у вас текущая профессия?',
    TARGET_PROFESSION: 'Какую профессию вы хотите получить?',
    EXPERIENCE: 'Какой у вас опыт в этой сфере? (От 1 до 13 световых лет):',
    ABOUT: 'Кратко расскажите о себе (макс. 400 символов):',
    AVATAR: '📷 Отправьте свое фото или нажмите "Пропустить" для продолжения:',
    SAVED: '✅ Профиль сохранён!',
    CONFIRM:
      'Проверьте, всё ли верно. Ответьте (y/n). Или начните заново /start.',
  },
  NOTIFICATION: {
    FOLLOW_INSTRUCTIONS: 'Пожалуйста, следуйте инструкциям.',
    EXIT: 'Выхожу...',
    SEARCH: 'Введите текст после @po9avan_bot для поиска:',
  },
  EDIT: {
    INTRO: 'Выберите, что хотите изменить.',
    NICK_APPLY: '✅ Ник обновлён.',
    ABOUT_APPLY: '✅ Текст «О себе» обновлён.',
    AVATAR_APPLY: '✅ Фото обновлено.',
    AVATAR_SKIPPED: 'Ок, фото оставим без изменений.',
  },
} as const;

export type ReplyKey = keyof typeof REPLIES;
